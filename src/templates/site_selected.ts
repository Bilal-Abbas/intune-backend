...
export function getSiteSelectedEmailHTML(
    ...
    <div style="font-family: Arial, sans-serif; padding: 20px; background: #f4f4f4;">
      <div style="max-width: 600px; margin: 0 auto; background-color: #ffffff; border-radius: 10px; box-shadow: 0px 0px 10px rgba(0, 0, 0, 0.1); padding: 32px; text-align: center;">
        ...
        <p style="font-size: 16px; margin-bottom: 24px; text-align: left;">
          We're excited to let you know that <strong>${sponsorName}</strong> has officially selected your site for participation in the study <strong>"${studyTitle}"</strong> on the Intune platform.
        </p>
        ...
        <p style="font-size: 16px; margin-bottom: 24px; text-align: left;">
          This is a major milestone in the site selection process. You can now coordinate directly with the sponsor to finalize activation steps, including contract execution, training, and site initiation.
        </p>

        <p style="font-size: 16px; margin-bottom: 24px; text-align: left;">
          Please log in to your account to view the current status, exchange documents, and message the sponsor if needed:
        </p>
...
export function getSiteSelectedEmailSubject(
  data: SiteSelectedTemplateData
): string {
  return `${data.sponsorName} Has Selected Your Site for "${data.studyTitle}" on Intune`;
}
